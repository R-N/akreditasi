...
class M_Guest extends CI_Model {
	...
	function fetch_dokumen($parent){
		$sql =  "SELECT * FROM dokumen_pendukung dp, ids WHERE dp.parent=? AND ids.id=dp.parent;";
		$query = $this->db->query($sql, array($parent));
		$result = $query->result_array();
	}
	function fetch_paragraph($parent){
		$sql =  "SELECT * FROM paragraf p, ids WHERE p.parent=? AND ids.id=p.parent;";
		$query = $this->db->query($sql, array($parent));
		$result = $query->result_array();
	}
